...
import hmm_utils_states_identified as utils
...
# Potential To-Do: Introduce a "Current mapping values" dict/df into the for loop below so that each function doesn't
#                   have to grab this info from the original mapping_dict on its own. Maybe even replace the whole
#                   "best_num_states" thing with the actual states and loop through them in each function.
mapping_dict = {
    '3p001_20241110': {
        1: 'Optimal',
        2: 'Sub-Optimal'
...
for idx, filename in enumerate(files):
    ...
    posterior_probs = utils.map_states_to_posterior_probs(posterior_probs, mapping_dict, mice_and_date, best_num_states)
    posterior_probs = utils.merge_identical_states(posterior_probs)
    best_num_states = len(posterior_probs['State'].unique())

    # Create the sequence of most probable states
    state_sequence = utils.create_state_sequence(data, best_num_states, posterior_probs, mapping_dict, mice_and_date)
    print(pd.DataFrame(state_sequence).to_string())
    ...
    utils.plot_state_probabilities(posterior_probs, hmm_data, cols, mice_and_date)
    ...
    pupil_diameter_df, hmm_df, dropped_pupil_trials, dropped_state_trials = utils.drop_outliers(pupil_diameter_df, hmm_data)
    after_outlier_removal_data = {
        'Trials': str(len(pupil_diameter_df)),
        'Min Diameter': pupil_diameter_df['Diameter'].min(),
        'Max Diameter': pupil_diameter_df['Diameter'].max(),
        'Avg Diameter': pupil_diameter_df['Diameter'].mean(),
        'Dropped Pupil Trials': dropped_pupil_trials,
        'Dropped State Trials': dropped_state_trials
    }
    ...
    utils.plot_binned_correlation_diameter_state(state_pupil_df, 5, cols, mice_and_date)