...
def map_states_to_posterior_probs(posterior_probs, mapping_dict, mice_and_date, best_num_states):
    ...
    for trial in range(len(posterior_probs)):
        for state in range(best_num_states):
            ...
            state_list.append(mapping_dict[mice_and_date][state + 1])
...
# This function achieves post-hoc merging of states that are deemed to be the same based on the behavioural metrics.
# If there are identical states for the same trial then the StateProbabilities are summed up and only one entry for that
# given state remains.
def merge_identical_states(posterior_probs):
    merged_states_df = posterior_probs.groupby(["Trial", "State"], as_index=False).agg({"StateProbability": "sum"})
    return merged_states_df


# Determining which state is the most likely for each trial and writing them into a sequence.
# If the probability is lower than 0.8 it will be assigned state 'Below Threshold' to signal that for that given trial
# no state is sufficiently likely.
# Potential To-Do: make the threshold of 0.8 an input variable
def create_state_sequence(data, best_num_states, posterior_probs, mapping_dict, mice_and_date):
    state_sequence = []
...
# Potential To-Do: Fix the "for k..." loop. Instead of "... .unique()" it should be "states" which are
#                   delivered from main
def plot_state_probabilities(posterior_probs, data, cols, mice_and_date):
    fig, ax = plt.subplots(figsize=(10, 5), dpi=80)

    # Plot state probabilities
    for state in posterior_probs['State'].unique():
        state_data = posterior_probs[posterior_probs["State"] == state]
        ax.plot(state_data["Trial"], state_data["StateProbability"], label=state, lw=2, color=cols[state])
    ...
    errors = np.where((data['Correct'] == 0) & (data['Decision'] != 0))[0]
...
def drop_outliers(pupil_df, hmm_df):
    ...
    dropped_pupils = len(rows_to_drop_pupil)
    pupil_df = pupil_df[(pupil_df['Diameter'] >= lower_bound) & (pupil_df['Diameter'] <= upper_bound)]
    hmm_df = hmm_df.drop(rows_to_drop_pupil)

    rows_to_drop_states = hmm_df[(hmm_df['State'] == 'Below Threshold')].index
    dropped_states = len(rows_to_drop_states)
    ...
    return pupil_df, hmm_df, dropped_pupils, dropped_states
...
def create_state_pupil_diameter_data(hmm_data, best_num_states, posterior_probs, diameter_df, mapping_dict, mice_and_date):
    rows = []
...
# in the figure.
def plot_binned_correlation_diameter_state(state_pupil_df, bin_width, cols, mice_and_date):
    for i, state in enumerate(state_pupil_df['State'].unique()):
        ...
        plt.scatter(binned_data['RelPupilDiameter'], binned_data['StateProbability'],
                    label=state, marker='o', color=cols[state])
